...
import br.com.zapia.wpp.api.model.handlersWebSocket.AbstractFindContactByNumberHandler;
...
@Component
@Scope("usuario")
public class FindContactByNumberHandler extends AbstractFindContactByNumberHandler<WebSocketRequestSession> {
    ...
    public CompletableFuture<WebSocketResponse> handle(WebSocketRequestSession webSocketRequestSession, String number) throws JsonProcessingException {
        return whatsAppClone.getWhatsAppClient().findContactByNumber(number).thenCompose(contact -> {
            if (contact == null) {
